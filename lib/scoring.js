...
// Where each signal comes from. Only cross-origin agreement is corroboration:
// rating and keyword-pain both read the same ~5 reviews Google chose to surface,
// so if that sample is unrepresentative they are wrong together. Two of them
// agreeing is one source speaking twice.
export const SIGNAL_ORIGIN = {
  confirmed_source: "public_record",      // an issued TABC licence
  card_present_processor: "site_fingerprint", // a POS SDK on their own site
  rating_dissatisfaction: "review_corpus",
  keyword_pain: "review_corpus",
  website_absent_confirmed: "review_corpus",
};

// The independent evidence that actually fired. Deliberately excludes every
// proxy the rest of this spec spent its time distrusting: an unlisted website,
// a review count on its own, a price level, and ambiguous or online-only
// processor hits.
export function collectSignals(b, weights) {
  const vc = weights.volume_confidence ?? DEFAULT_VOLUME_CONFIDENCE;
  const signals = [];

  if (b.source === "tabc") signals.push("confirmed_source");

  const top = bestProcessor(b.processor);
  if (top && tierOf(top) === "card_present") signals.push("card_present_processor");

  // Full volume confidence only. A thin 10–19-review rating scores at half
  // weight but corroborates nothing.
  if (b.rating !== null && b.rating !== undefined && b.rating <= 4.2 &&
      volumeConfidence(b.review_count, vc) === 1) {
    signals.push("rating_dissatisfaction");
  }

  if (painHits(b.review_texts).length > 0) signals.push("keyword_pain");

  if (websiteStatus(b.website, b.review_texts) === "absent_confirmed") {
    signals.push("website_absent_confirmed");
  }

  return signals;
}

// Computed from the signal list alone — never from the score. A Hot score built
// entirely from proxies is exactly the case this exists to catch.
export function computeConfidence(signals) {
  if (!signals || signals.length === 0) return "low";
  const hasNonReview = signals.some((s) => SIGNAL_ORIGIN[s] !== "review_corpus");
  if (signals.length >= 2 && hasNonReview) return "high";
  // Covers one signal of any strength, and two-or-more that are all review-derived.
  return "medium";
}
...
export function scoreBusiness(b, weights, icp) {
  ...
  if (track === "displacement") {
    ...
    // A TABC row is a licence the state actually issued. A Places row is a guess
    // off a low review count. They are not the same claim and must not read the
    // same — this was the review's point #6.
    if (b.source === "tabc") {
      why.push(`confirmed new — TABC license issued ${b.licensed_on ?? "recently"}`);
    } else {
      why.push(`${b.review_count} reviews (new — inferred, not confirmed)`);
    }
  ...
  // Confidence is a second, orthogonal axis: the score says how good the lead
  // looks, this says how much of that to believe. Derived from the signal list
  // only — business_status deliberately does not cap it, because a temporarily
  // closed business's evidence is exactly as strong as it was. The warn chip
  // above is what says "don't dial today".
  const signals = collectSignals(b, weights);
  const confidence = computeConfidence(signals);

  score = Math.max(0, Math.min(100, pyRound(score)));
  return scoredLead({
    business: b, track, score, bucket: bucketFor(score, weights.buckets), why, signals, confidence,
  });
}
